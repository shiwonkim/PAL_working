...
(repo 루트)
  run_pipeline.sh   extract -> train -> eval 체인 실행기 (config를 $1로 받음)
  rerun_eval.py     eval 진입점
  viz/              해석용 시각화 / 논문 figure (독립 실행, src를 라이브러리로 import)
```
...
| `3b3ccef` | `scripts/viz/` → 루트 `viz/`로 분리 (해석용 시각화; sys.path/옛경로 보정) |
| `efd4c50` | legacy 체크포인트 마이그레이션 코드 청산 (checkpoint.py 슬림화 + migrate/verify 삭제) |
| `e8d4c38` | `scripts/` 전체 제거, `run_pipeline.sh` → repo 루트 (실험 런처는 sweep용이라 불필요) |
...
3. ~~`scripts/` 정리~~ ✅ 완료 (`e8d4c38`: 전체 제거 + run_pipeline.sh 루트로).

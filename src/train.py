...
def load_dataset(
    ...
    if dataset_name not in ("coco", "coco2017", "flickr30", "pathcap"):
...
def run(
    ...
    Trains only (no evaluation): features are loaded from cache or extracted on
    a miss (FeatureStore extract-or-load), the alignment layers are trained, and
    a checkpoint is saved. Run ``src/eval.py`` afterwards to evaluate it.
    """

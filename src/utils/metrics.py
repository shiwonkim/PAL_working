...
class AlignmentMetrics:
    ...
    @staticmethod
    def hcs(feats_A, feats_B, levels=3, tau=0.05, eps=1e-8):
        # X, Z: [n,d] (already on GPU / batched if needed)
        Xn = (feats_A / (feats_A.norm(dim=1, keepdim=True) + eps)) - feats_A.mean(
            0, keepdim=True
        )
        Zn = (feats_B / (feats_B.norm(dim=1, keepdim=True) + eps)) - feats_B.mean(
            0, keepdim=True
        )
        Sx = (Xn @ Xn.T) / tau  # cosine similarities
        Sz = (Zn @ Zn.T) / tau
        Px = torch.softmax(Sx, dim=1)
        Pz = torch.softmax(Sz, dim=1)

        js_total = 0.0
        Px_l, Pz_l = Px, Pz
        for level in range(1, levels + 1):
            if level > 1:  # fast power
                Px_l = Px_l @ Px
                Pz_l = Pz_l @ Pz
            M = 0.5 * (Px_l + Pz_l) + eps
            js = 0.5 * (
                (Pz_l * torch.log(Pz_l / M)).sum() + (Px_l * torch.log(Px_l / M)).sum()
            )
            js_total += js / level
        R_s = js_total / levels
        return torch.exp(-R_s).item()  # similarity in [0,1]
    ...
    def l_hop_cka(
        ...
        for level in range(1, levels + 1):
            ...
            hsic_kk = hsic_fn(K_l, K_l)
            hsic_ll = hsic_fn(L_l, L_l)
            hsic_kl = hsic_fn(K_l, L_l)
            # Compute CKA
            cka_total += (hsic_kl / (torch.sqrt(hsic_kk * hsic_ll) + 1e-6)) / level
        cka_total = cka_total / levels
        return cka_total.item()

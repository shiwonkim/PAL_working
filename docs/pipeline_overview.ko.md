...
| **PAL-CLS** | `PALAlignmentLayer` (`pal.py`, 2D 경로) | `normalize(normalize(z) @ normalize(anchors)ᵀ)` → `(B,K)` 코사인 profile |
| **PAL-Token ★** | `PALAlignmentLayer` (`pal.py`, 3D 경로) | **CAP**, 아래 참고 |

> PAL-CLS와 PAL-Token은 **같은** `PALAlignmentLayer`; forward가 입력 rank(2D CLS vs
> 3D 토큰)로 경로를 고름. config `token_level`이 무엇을 먹일지 결정.
...
5. **CAP 레이어**(`pal.py`)는 비교적 깔끔; cls_attn-prior 분기가

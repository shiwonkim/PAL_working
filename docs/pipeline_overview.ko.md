...
| **BA-CLS** | `BridgeAnchorAlignmentLayer` (`bridge_anchor.py` ~57) | `normalize(normalize(z) @ normalize(anchors)ᵀ)` → `(B,K)` 코사인 profile |
| **BA-Token ★** | `BridgeAnchorTokenAlignmentLayer` (`bridge_anchor_token.py` ~116) | **CAP**, 아래 참고 |
...
2D(CLS) 입력이면 BA-CLS 경로로 fallback (~124-128). 이게 리팩터 대상 레이어이고,
...
| 메소드 | Linear / MLP / CSA / BA-CLS | BA-Token / FreezeAlign |
...
5. **CAP 레이어**(`bridge_anchor_token.py`)는 비교적 깔끔; cls_attn-prior 분기가
